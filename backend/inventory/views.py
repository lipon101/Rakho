...
class BootstrapAdminView(APIView):
    """One-time superuser creation for free-tier hosts without a Shell.

    Render's free plan has no interactive shell, so `createsuperuser` cannot be
    typed. This endpoint exists only to bootstrap the very first admin account:

    - Requires the secret SETUP_TOKEN (sent as the X-Setup-Token header), which
      only the owner can see in the Render dashboard.
    - Self-disables permanently once any superuser exists, so it cannot be used
      to take over an already-set-up deployment.
    - Only ever runs in production mode (DEBUG=false).

    Delete this view after the first login if you want it gone entirely.
    """
    authentication_classes = []
    permission_classes = [permissions.AllowAny]
    throttle_scope = "signup"
